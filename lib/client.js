...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var PRECISION = 2;
    var TURN_DECIMALS = 4; // 本轮 stays fine-grained (估算/跳动可见)
    ...
    function buildGroups(stats, usage, turnCny, totalCny, balance, meta) {
      ...
      if (balance !== null) {
        ...
        if (balance.amount !== null) {
          ...
          if (balanceCriticalCny > 0 && balAmount <= balanceCriticalCny) {
            ...
            balWarn = "⚠ ";
            balPop += "\n余额告警：低于 " + fmtMoney(currencySymbol(balance.currency), balanceCriticalCny) + "（红色）";
          } else if (balanceWarnCny > 0 && balAmount <= balanceWarnCny) {
            balStyle = { color: "#f59e0b" };
            balWarn = "⚠ ";
            balPop += "\n余额告警：低于 " + fmtMoney(currencySymbol(balance.currency), balanceWarnCny) + "（琥珀）";
          }
      ...
      if (usage !== void 0) {
        ...
        // 本轮 at 4 decimals (small amounts, movement visible), 会话 at 2
        // (reconciles against the balance)
        var spendText = "本轮 " + fmtMoney("¥", totalShown, TURN_DECIMALS) +
