    ...
    function buildGroups(stats, usage, turnCny, totalCny, balance, meta) {
      ...
      if (usage !== void 0) {
        ...
        if (estimateCny > 0 || sessionRunning) {
          popLines.push("本轮 " + fmtMoney("¥", totalShown) + "（精确 " + fmtMoney("¥", turnCny) + " + 估算 " + fmtMoney("¥", estimateCny) + "）");
        } else {
          popLines.push("本轮 " + fmtMoney("¥", turnCny));
        }
        popLines.push("会话 " + approx + fmtMoney("¥", sessionShown) + (unpricedSteps > 0 ? "（含 " + unpricedSteps + " 步未定价 · 模型未知）" : ""));
