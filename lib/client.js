...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
      ...
      if (stats) {
        ...
        if (stats.decodeMs > 0) speeds.push(formatTps(stats.decodeTokens / (stats.decodeMs / 1000)));
        {
          ...
          if (openSp !== null && openSp.firstTokenTime !== null && typeof openSp.firstTokenTime === "number") {
            var fw = meta !== null && meta !== void 0 && typeof meta.firstTokWall === "number" ? meta.firstTokWall : null;
            var lw = meta !== null && meta !== void 0 && typeof meta.lastTokWall === "number" ? meta.lastTokWall : null;
            ...
            if (openSp.replayed !== true && fw !== null && lw !== null && lw - fw > 0) {
              ...
              var le = meta !== null && meta !== void 0 && typeof meta.lastTokEvt === "number" ? meta.lastTokEvt : null;
              if (le !== null) {
    ...
    function SessionStats(props) {
          ...
          fetch("/plugins/better-stats/live?sessionId=" + encodeURIComponent(sessionId), { cache: "no-store" })
            ...
            .then(function (body) {
              ...
              setLiveInfo({
                ...
                unpricedSteps: Number(body.unpricedSteps) || 0,
                invalidSteps: Number(body.invalidSteps) || 0,
                seedLength: typeof body.seedLength === "number" && Number.isFinite(body.seedLength) ? body.seedLength : null,
                pricing: body.pricing && typeof body.pricing === "object" ? body.pricing : null,
          ...
          if (estLen >= estState.next) {
            ...
            try {
              if (liveInfo !== null && liveInfo !== void 0 && typeof liveInfo.seedLength === "number") {
                ...
                if (sessHdr !== void 0 && sessHdr !== null && typeof sessHdr.seedLength === "number" && sessHdr.seedLength > 0) {
                  sessSeed = Math.floor(sessHdr.seedLength);
                } else {
                  ...
                  if (sessRow === void 0 || sessRow === null || sessRow.parentId === void 0 || sessRow.parentId === null || sessRow.parentId === "") {
                    sessSeed = 0;
                  }
                }
              }
            } catch (e) { sessSeed = 0; }
            // the seed boundary arrived/changed: rebuild the session fold
            // from scratch (fresh map + cost), scanning only post-seed events
            if (sessSeed !== null && estState.sessSeed !== sessSeed) {
              estState.sessSeed = sessSeed;
              sessSamples = new Map();
              sessCost = 0;
              var seedModel = estState.lastModel !== void 0 ? estState.lastModel : currentModel;
              for (var sei = sessSeed; sei < estLen; sei++) {
                var sev = clientEvents[sei];
                if (sev === void 0 || sev === null || typeof sev !== "object") continue;
                var sed = sev.data;
                if (sed === void 0 || sed === null) continue;
                if (sev.type === "assistant/chunk" && sed.chunk !== void 0 && sed.chunk !== null && sed.chunk.type === "usage" && sed.chunk.usage !== void 0 && sed.chunk.usage !== null) {
                  sessCost = upsertTurnSample(sessSamples, sessCost, sed.turn, sed.step, sed.chunk.usage, seedModel, sev.time, effectiveTables, null);
                } else if (sev.type === "assistant/message" && sed.usage !== void 0 && sed.usage !== null) {
                  var sem = sed.message && sed.message.source && typeof sed.message.source.model === "string" ? sed.message.source.model : void 0;
                  if (typeof sem === "string" && sem !== "") seedModel = sem;
                  sessCost = upsertTurnSample(sessSamples, sessCost, sed.turn, sed.step, sed.usage, sem, sev.time, effectiveTables, null);
                }
              }
            }
            ...
            function touchTokWall(evtTime, replayEvt) {
              ...
              if (typeof evtTime === "number") estState.lastTokEvt = evtTime;
            }
              ...
              if (ev.type === "turn/start") {
                ...
                if (ck.type === "usage" && ck.usage !== void 0 && ck.usage !== null) {
                  ...
                  if (sessSeed !== null && ei >= sessSeed) {
                    sessCost = upsertTurnSample(sessSamples, sessCost, ev.data.turn, ev.data.step, ck.usage, lastModel, ev.time, effectiveTables, null);
                  }
                  sessionModelUpsert(ev);
                ...
                if (ev.data.usage !== void 0) {
                  ...
                  if (sessSeed !== null && ei >= sessSeed) {
                    sessCost = upsertTurnSample(sessSamples, sessCost, ev.data.turn, ev.data.step, ev.data.usage, msgModel, ev.time, effectiveTables, null);
                  }
                  sessionModelUpsert(ev);
      ...
      // The client's own exact whole-session fold (sessCost — the same usage
      // events the host folds, priced per sample at event time) is NEVER
      // stale, so the FIRST round shows 会话 == 本轮 from the settle moment
      // instead of a lagging host poll (1s /live, 10s /cost cache). A legal
      // zero root is still a real answer (the client fold is also 0 then) —
      // this only prefers the fresher EXACT figure, never a guess.
      ...
      var sessionCost;
      if (rootSettled !== null && rootSettled >= clientSessCost) {
        // host snapshot is at least as fresh as the client fold — authoritative
        sessionCost = rootSettled + snapDescCost;
      } else if (clientEvents !== null && clientSessCost > 0) {
        // the client's exact real-time fold is fresher than the host polls
        sessionCost = clientSessCost + snapDescCost;
      } else {
        sessionCost = rootSettled !== null ? rootSettled + snapDescCost : liveCostNow;
      }
