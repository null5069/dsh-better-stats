  ...
  factory: function (require) {
    ...
    function buildGroups(stats, usage, turnCny, totalCny, balance, meta) {
      ...
      if (balance !== null) {
        ...
        if (balance.amount !== null) {
          ...
          if (balance.granted != null || balance.toppedUp != null) {
            var splitParts = [];
            if (balance.granted != null) splitParts.push("赠送 " + fmtMoney(currencySymbol(balance.currency), balance.granted, balDec));
            if (balance.toppedUp != null) splitParts.push("充值 " + fmtMoney(currencySymbol(balance.currency), balance.toppedUp, balDec));
          ...
          if (balanceCriticalCny > 0 && balAmount <= balanceCriticalCny) {
            ...
            balWarn = "⚠ ";
            balPop += "\n余额告警：低于 " + fmtMoney(currencySymbol(balance.currency), balanceCriticalCny, CONFIG_DECIMALS) + "（红色）";
          } else if (balanceWarnCny > 0 && balAmount <= balanceWarnCny) {
            balStyle = { color: "#f59e0b" };
            balWarn = "⚠ ";
            balPop += "\n余额告警：低于 " + fmtMoney(currencySymbol(balance.currency), balanceWarnCny, CONFIG_DECIMALS) + "（琥珀）";
          }
          groups.push({ label: "余额", text: balWarn + "余额 " + balText, value: balText, popover: balPop, style: balStyle });
        } else {
      ...
      if (meta !== null && meta !== void 0 && meta.peakGroup !== null && meta.peakGroup !== void 0) {
        ...
        groups.push({
          label: "峰谷",
          text: pg.peak ? "高峰中" : "空闲中",
          popover: (pg.peak ? "高峰中（价格×2）" : "空闲中") + " · " + pg.label + "（" + pg.minutesLeft + " 后）"
        });
      }
      if (usage !== void 0) {
        ...
        var spendText = "本轮 " + fmtMoney("¥", totalShown) +
          " · 会话 " + approx + fmtMoney("¥", sessionShown);
        var popLines = [];
        ...
        if (estimateCny > 0 || sessionRunning) {
          popLines.push("本轮 " + fmtMoney("¥", totalShown, POPOVER_DECIMALS) + "（精确 " + fmtMoney("¥", turnCny, POPOVER_DECIMALS) + " + 估算 " + fmtMoney("¥", estimateCny, POPOVER_DECIMALS) + "）");
        } else {
          popLines.push("本轮 " + fmtMoney("¥", turnCny, POPOVER_DECIMALS));
        }
        popLines.push("会话 " + approx + fmtMoney("¥", sessionShown, POPOVER_DECIMALS) + (unpricedSteps > 0 ? "（含 " + unpricedSteps + " 步未定价 · 模型未知）" : ""));
    ...
    function BetterStatsLineInner(props) {
      ...
      if (hostPricing !== null && hostPricing.source === "official") {
        pricingSourceText = "DeepSeek 官方 " + beijingDateLabel(hostPricing.fetchedAt);
      } else if (hostPricing !== null && hostPricing.source === "stale") {
        pricingSourceText = "DeepSeek 官方价目(已过期)";
      }
      ...
      if (budget !== null && todayCost !== null) {
        ...
        if (dailyBudget > 0) {
          ...
          budgetLines.push("今日 " + fmtMoney("¥", todayCost.costCny) + " · 日预算 " + fmtMoney("¥", dailyBudget, CONFIG_DECIMALS) + " (" + Math.round(dayPct * 100) + "%)");
        ...
        if (monthlyBudget > 0 && todayCost.monthCostCny !== null) {
          ...
          budgetLines.push("本月 " + fmtMoney("¥", todayCost.monthCostCny) + " · 月预算 " + fmtMoney("¥", monthlyBudget, CONFIG_DECIMALS) + " (" + Math.round(monthPct * 100) + "%)");
                ...
                if (group.label === "花费" && pricingSourceText !== "") {
                  rows.push(
                    react.createElement(
                      ...
                      react.createElement("span", { className: "dsh-better-stats-pop-label" }, "价源"),
                      pricingSourceText
                    )
                  );
                }
