...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function BetterStatsLineInner(props) {
      ...
      // Streaming estimate: the live log's *-delta chunk events are only a
      // tiny sample — the full streamed text arrives in the batch events
      // (reasoning-chunks / text-chunks / tool-call-chunks). Count their
      // chars since the last usage event, convert at per-kind density, price
      // at the current model + current tier, and add the carried input cost
      // of the current step. Only while a step is open AND the session is
      // running; the step's usage chunk resets the counters, so the estimate
      // never double-counts (display-only — turnCostRef stays exact).
      var estimateCny = 0;
      if (sessionRunning && liveInfo !== null && liveInfo.openStepStart !== null && liveInfo.openStepStart !== void 0 && clientEvents !== null) {
        try {
          ...
          if (estLen >= estState.next) {
            ...
            for (var ei = estState.next; ei < estLen; ei++) {
              ...
              if (ev.type === "reasoning-chunks" && ev.data !== void 0 && Array.isArray(ev.data.texts)) {
                ...
                if (ck.type === "usage" && ck.usage !== void 0 && ck.usage !== null) {
                  // step settled: exact figures take over, carry the input
                  // snapshot for the NEXT step's input estimate
                ...
                // consecutive steps of a session)
                reason.cjk = 0; reason.rest = 0;
                text.cjk = 0; text.rest = 0;
                tool.cjk = 0; tool.rest = 0;
          ...
          var estState2 = estimateRef.current;
          var estOutPrice = 0;
          ...
          var estTokens =
            (estState2.reason.cjk + estState2.reason.rest / EST_DENSITY_REASON) +
            (estState2.text.cjk + estState2.text.rest / EST_DENSITY_TEXT) +
            (estState2.tool.cjk + estState2.tool.rest / EST_DENSITY_TOOL);
          estimateCny = estTokens * estOutPrice / 1e6 + estState2.inputCny;
        } catch (e) {
          estimateCny = 0;
        }
      }
