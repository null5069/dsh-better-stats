...
function priceBuckets(buckets, time, model, tables) {
  ...
  return (
    ((buckets.uncachedInputTokens + buckets.cacheWriteTokens) * miss +
      ...
      (buckets.outputTokens + (buckets.reasoningTokens || 0)) * out) / 1e6
  );
}
...
function foldUsage(events, opts) {
  ...
  // In-flight usage chunks carry NO model; attribute them to the model this
  // turn last settled with, so the live per-model figures tick with the open
...
async function queryTreeCost(ctx, persistence, sessionId, signal) {
  ...
  const merged = { uncachedInputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, outputTokens: 0, reasoningTokens: 0 };
  let costCny = 0;
  let unpricedSteps = 0;
    ...
    try {
      ...
      if (Array.isArray(events) && events.length > 0) {
        ...
        for (const [key, entry] of folded.byModel.entries()) {
          const acc = byModel.get(key) ?? { usage: { uncachedInputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, outputTokens: 0, reasoningTokens: 0 }, costCny: 0 };
...
export function apply(ctx, config = {}) {
  ...
  // Optional budget/alert config from the bundle patch row
  // (cordis.patch.yml: config: { dailyBudgetCny, monthlyBudgetCny,
  // balanceWarnCny, balanceCriticalCny }). Daily/monthly budgets are OFF
  // unless configured. Balance alerts have two tiers: warn (amber, default
  // ¥20) and critical (red, default ¥5); set a tier to 0 to disable it.
  const budgetPayload = (() => {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      // ?force=1 bypasses the cache (click-to-refresh from the UI), with a
      // 2s anti-flood cooldown so a misbehaving client cannot hammer upstream.
        ...
        try {
          ...
          if (!hit) {
            ...
            if (force && now - lastForceAt < FORCE_COOLDOWN_MS && cached !== null) {
              // Cooldown: serve the cache instead of a second forced query.
              body = { ...cached.data, queriedAt: new Date(cached.at).toISOString() };
            } else {
