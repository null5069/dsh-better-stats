...
//             displayName, amount, currency, grantedBalance, toppedUpBalance,
//             queriedAt }
...
let cached = null; // { at, data } — last successful balance snapshot
let lastForceAt = 0; // last ?force=1 query (anti-flood cooldown)
...
function ensurePricing() {
  ...
  const backedOff = pricingState.tables === null && now - pricingState.lastErrorAt < PRICING_RETRY_MS;
...
function pricingPayload() {
  if (pricingState.tables === null) {
    return { source: "builtin", fetchedAt: null, tables: BUILTIN_TABLES, version: 0 };
  }
        ...
        if (open !== null && e.data !== void 0 && open.turn === e.data.turn && open.step === e.data.step) {
          ...
          if (open.firstTokenTime !== null) {
            ...
            if (out !== null) {
              ...
              state.decodeTokens += out; // output already includes reasoning
            }
          }
...
function indexHeaders(liveList, persistedHeaders, target) {
  const byId = target !== void 0 && target !== null ? target : {};
  const add = (id, header) => {
...
function collectDescendantIds(byId, rootId) {
  const out = [];
  const reachable = new Set([rootId]); // sessions linked into the tree
  ...
  while (grew) {
    ...
    for (const id in byId) {
      ...
      const entry = byId[id];
      if (entry !== void 0 && entry.origin === "subagent" && reachable.has(entry.parentId)) {
        reachable.add(id);
        seen.add(id);
        out.push(id);
        grew = true;
...
async function queryBalance(key) {
  ...
  if (!Number.isFinite(amount) || amount < 0) throw new Error("provider returned an invalid balance");
  const toNumOrNull = (v) => {
    const n = Number(v);
...
async function sessionEvents(ctx, persistence, id, signal) {
  ...
  if (live !== void 0 && Array.isArray(live.events)) {
    return { events: live.events, meta: live.header !== void 0 && live.header !== null ? live.header : {} };
  }
...
async function queryTreeCost(ctx, persistence, sessionId, signal) {
  const snapshot = pricingSnapshot(); // immutable, captured at request start
  const byId = indexHeaders(ctx.sessions.list(), []);
  ...
  if (!(sessionId in byId)) {
    const live = ctx.sessions.get(sessionId);
    if (live !== void 0) {
      byId[sessionId] = {
        parentId: live.header && live.header.parentSession,
        origin: live.header && live.header.origin === "subagent" ? "subagent" : "other",
        seedLength: typeof (live.header && live.header.seedLength) === "number" ? live.header.seedLength : 0
      };
    } else {
...
let todayCache = null; // { at, key, data }
...
async function queryToday(ctx, persistence, signal) {
  ...
  const liveList = (() => {
    try { return ctx.sessions.list(); } catch { return []; }
  })();
...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const cachedData = () => ({ ...cached.data, queriedAt: new Date(cached.at).toISOString() });
      if (cached !== null && !force && now - cached.at < CACHE_TTL_MS) {
      ...
      try {
        ...
        if (!hit) {
          write({ configured: false, status: "ok", provider: null, displayName: null, amount: null, currency: null, grantedBalance: null, toppedUpBalance: null, queriedAt: new Date().toISOString() });
          return;
        }
        if (force && now - lastForceAt < FORCE_COOLDOWN_MS && cached !== null) {
          write({ ...cachedData(), status: "stale" }); // cooldown: serve cache
          return;
        }
        if (force) lastForceAt = now;
        const data = await singleflight("balance", () => queryBalance(hit.value));
        cached = { at: now, data: { configured: true, ...data } };
        write({ configured: true, status: "ok", ...data, queriedAt: new Date(now).toISOString() });
      } catch {
        if (cached !== null) {
          write({ ...cachedData(), status: "stale" }); // stale-while-error
        } else {
          write({ configured: true, status: "error", error: "unavailable", provider: null, displayName: null, amount: null, currency: null, grantedBalance: null, toppedUpBalance: null, queriedAt: new Date().toISOString() });
        }
      }
    }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const key = sessionId + "|" + pricingSnapshot().version;
      ...
      try {
        const result = await singleflight("cost:" + sessionId, () =>
          queryTreeCost(ctx, persistence, sessionId, AbortSignal.timeout(15000)));
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (req, res) => {
      ...
      const seed = session.header !== void 0 && session.header !== null &&
        ...
        : 0;
      const live = foldLive(session.events, { startIndex: seed });
      let rootCostCny = 0;
      let rootModels = [];
      let unpricedSteps = 0;
      let invalidSteps = 0;
      try {
        const usage = foldUsage(session.events, { snapshot: pricingSnapshot(), startIndex: seed });
        rootCostCny = usage.costCny;
        unpricedSteps = usage.unpricedSteps;
        invalidSteps = usage.invalidSteps;
        rootModels = [...usage.byModel.entries()].map(([model, entry]) => ({ model, usage: entry.usage, costCny: entry.costCny }));
      } catch {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const key = date + "|" + date.slice(0, 7) + "|" + pricingSnapshot().version;
      ...
      try {
        const result = await singleflight("today", () => queryToday(ctx, persistence, AbortSignal.timeout(15000)));
        todayCache = { at: now, key, data: result };
