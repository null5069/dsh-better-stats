...
// Session logs in the wild contain both the historical message envelope and
// a flat message projection. Keep every fold compatible with both shapes.
...
function messageSourceOf(data) {
  ...
  const body = messageBodyOf(data);
  return body !== null && body.source !== void 0 && body.source !== null && typeof body.source === "object"
    ? body.source
    : null;
}

function messageContentOf(data) {
  if (data === void 0 || data === null || typeof data !== "object") return null;
  if (Array.isArray(data.content)) return data.content;
  const body = messageBodyOf(data);
  return body !== null && Array.isArray(body.content) ? body.content : null;
}
...
function foldUsage(events, opts) {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    const messageSource = e.type === "assistant/message" ? messageSourceOf(e.data) : null;
    const sourceModel = messageSource !== null ? messageSource.model : void 0;
    if (i < startIndex) {
    ...
    } else if (e.type === "assistant/message" && e.data && messageUsageOf(e.data) !== null) {
      turn = e.data.turn;
      step = e.data.step;
      usage = messageUsageOf(e.data);
      model = sourceModel;
    } else {
      continue;
    }
...
function foldLive(events, opts) {
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    switch (e.type) {
      ...
      case "assistant/message": {
        ...
        if (open !== null && data !== void 0 && data !== null && typeof data === "object" &&
          ...
          if (open.firstTokenTime !== null) {
            ...
            state.ttftSteps += 1;
            const validUsage = usageBucket(messageUsageOf(data));
            if (validUsage !== null) {
          ...
          const blocks = messageContentOf(data);
      ...
      case "tool/result": {
        const resultSource = messageSourceOf(e.data);
        const callId = resultSource !== null && typeof resultSource.callId === "string"
          ? resultSource.callId
          : (e.data && typeof e.data.callId === "string" ? e.data.callId : void 0);
