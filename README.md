...
- **本轮**：**按整轮（turn）结算**——精确部分 = 当前轮已落定步骤的事件级折叠（每步按各自事件时刻/模型计价，含 reasoning 输出计价），流式部分 = 当前步的字符级估算（**按当前峰谷时段计价**，密度由已结算步骤**自适应校准**：EMA 追踪真实字符/token 比）；多步 turn 内数字连续增长、不随 thinking 结束重算，usage 落地自动转精确，估算只进显示不入账；**一轮结束后本轮保留最终值，下一轮开始才归零**；行内不显示 (估) 后缀，精确/估算明细只在浮窗
- **会话**：host 端按完整轨迹逐步结算（每秒），**子代理整树合并**（含持久化旧会话），**实时跳动**（精确结算 + 当前步流式估算），浮窗显示 `（历史 + 本轮）` 分账（历史 = 本轮之外的精确，本轮 = 本轮精确 + 估算），悬浮层显示分模型明细与子会话数
...
- **余额告警（两档，默认 ≤¥20 琥珀 / ≤¥5 红色）**：余额组变色加 `⚠`，悬浮层提示；`config: { balanceWarnCny, balanceCriticalCny }` 调整阈值，对应档设 `0` 关闭
- **峰谷栏**：独立「峰谷」组常驻显示当前时段与下次切换（如 `高峰中 · 空闲 14:00 开始`），悬浮层显示详情（`高峰进行中（价格×2） · 空闲 14:00 开始（3h 20m 后）`，每秒随 /live 轮询刷新）
...
- **浮窗**：label 列对齐（多行内容等宽缩进）；本轮显示总数并在括号内注明明细 `本轮 ¥0.22（精确 ¥0.18 + 估算 ¥0.04）`（无估算时不带括号）；峰谷行内仅 `高峰中/空闲中`，详情（价格×2、下一阶段开始时间与倒计时）在浮窗内
