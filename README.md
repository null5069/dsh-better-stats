...
- **会话**：host 端按完整轨迹逐步结算（每秒），**子代理整树合并**（仅 `origin: subagent` 进入父会话树，普通 fork 独立成树；每个会话只折叠自己 `seedLength` 之后的事件，今日/本月汇总同样排除继承 seed）；**实时跳动且与本轮同源**——客户端以同一份事件做整会话精确折叠（含估算），在 host 轮询（1s /live、10s /cost 缓存）未追平时取**最新精确值**（首轮落定瞬间 会话 = 本轮，绝不再出现 会话 ¥0.0000 滞后），host 追平后仍以快照为准；悬浮层显示分模型明细与子会话数
